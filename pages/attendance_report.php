...
<head>
    ...
    <style>
        /* Style for the page-title class */
        .page-title {
            color: #007BFF; /* Set text color to a different shade of blue */
            font-size: 24px; /* Adjust the font size */
            text-align: center; /* Center the text */
            font-family: Arial, sans-serif; /* Set font style */
            margin: 20px 0; /* Add margin for spacing */
            text-transform: uppercase; /* Transform text to uppercase */
            background: linear-gradient(to right, #007BFF, #00BFFF); /* Optional gradient background */
            -webkit-background-clip: text; /* Make text transparent for gradient */
            -webkit-text-fill-color: transparent; /* Make text transparent for gradient */
        }

        hr {
            border: 1px solid #000; /* Style for the horizontal line */
        }
...
<body>
    ...
    if (!empty($class_month)) {
        $monthLastDay = date("t", strtotime("{$class_month}-01")); // Get last day of the month
    }
    ?>
                                                        ...
                                                        <td class="text-center px-2 py-1 text-dark-emphasis">
                                                            <?php 
                                                                ...
                                                                switch ($row['attendance'][$class_month . "-" . $i] ?? '') {
                                                                    case 1: echo "<span class='text-success fw-bold'>P</span>"; $tp++; break;
                                                                    case 2: echo "<span class='text-body-emphasis fw-bold'>L</span>"; $tl++; break;
                                                                    case 3: echo "<span class='text-danger fw-bold'>A</span>"; $ta++; break;
                                                                    case 4: echo "<span class='text-primary fw-bold'>H</span>"; $th++; break;
                                                                }
                                                            ?>
                                                        </td>
                                                    <?php endfor; ?>
    ...
    <script>
        ...
        $(document).ready(function(){
            $('#class_id, #class_month').change(function(){
